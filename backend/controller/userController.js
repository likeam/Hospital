...
export const login = catchAsyncErrors(async (req, res, next) => {
  ...
  if (password !== confirmPassword) {
    return next("Password & Confirm Password Do Not Match!", 400);
  }
  ...
  if (!isPasswordMatch) {
    return next("Invalid Eamil or Password", 400);
  }

  if (role !== user.role) {
    return next(`User Not Found with This Role!`, 400);
  }
...
export const addNewDoctor = catchAsyncErrors(async (req, res, next) => {
  ...
  if (isRegistered) {
    next(new ErrorHandler("Doctor with same Email already Exists", 400));
  }
  ...
  if (!cloudinaryRespone || cloudinaryRespone.error) {
    console.error(
      "Cloudinary Error: ",
      cloudinaryRespone.error || "Unknown Cloudinary error"
    );
    return next(
